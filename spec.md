...
To this json encoding corresponds exactly one [legacy data value](TODO), obtained by decoding this json with the [transport decoder](TODO). This value is referred to as a *legacy message*.
...
To compute the hash of a legacy message, use the [legacy value hash computation](TODO), with the signing encoding where the object entry order that would produce the correct signature.

##### Computing the Size of a Legacy Message

To compute the size of a legacy message, use the [legacy value length computation](TODO), with the signing encoding where the object entry order that would produce the correct signature.
